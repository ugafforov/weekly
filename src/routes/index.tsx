...
import { FileDown, FileSpreadsheet, ImageDown, LoaderCircle, Pencil, Upload, Users } from "lucide-react";
...
function RatingDashboard() {
  ...
  async function downloadImage() {
    ...
    try { const { toPng } = await import("html-to-image"); const url = await toPng(reportRef.current, { pixelRatio: 3, cacheBust: true, backgroundColor: "#ffffff" }); const link = document.createElement("a"); link.download = `${activeClass}-${workbook?.date}-reyting.png`; link.href = url; link.click(); }
    finally { setBusy(undefined); }
  }
  async function downloadPdf() {
    if (!reportRef.current || activeClass === "all") return; setBusy("pdf");
    try { const [{ toPng }, { jsPDF }] = await Promise.all([import("html-to-image"), import("jspdf")]); const image = await toPng(reportRef.current, { pixelRatio: 2, backgroundColor: "#ffffff" }); const pdf = new jsPDF({ orientation: "landscape", unit: "mm", format: "a4" }); const props = pdf.getImageProperties(image); const width = 277; pdf.addImage(image, "PNG", 10, 10, width, Math.min(190, width * props.height / props.width), undefined, "FAST"); pdf.save(`${activeClass}-${workbook?.date}-reyting.pdf`); }
    finally { setBusy(undefined); }
  }
  ...
  return <main className="min-h-screen px-4 py-5 lg:px-8">
    ...
    {!workbook ? <UploadScreen busy={busy} error={error} onChoose={() => inputRef.current?.click()} /> : <div className="mx-auto max-w-[1840px] py-6">
      ...
      <Report ref={reportRef} workbook={workbook} activeClass={activeClass} students={students} columns={columns} thirdSubject={thirdSubject} />
    </div>}
  </main>;
}

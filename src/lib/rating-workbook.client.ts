...
export async function parseRatingWorkbook(file: File): Promise<RatingWorkbook> {
  ...
  if (headerRow < 0)
    throw new Error("Could not locate the student name and class headers on the active sheet.");
  ...
  if (nameCol === undefined || classCol === undefined || totalCol === undefined) {
    throw new Error(
      "Required columns (name, class, and 5-Hafta) were not found among visible columns.",
    );
  }
  ...
  const columns = dataCols.map((col) => {
    const label =
      text(mergedValue(sheet, headerRow, col)) || `Column ${XLSX.utils.encode_col(col)}`;
    let group = "Natijalar";
  ...
  for (let row = range.s.r; row < headerRow && !date; row += 1) {
    for (const col of visibleCols) {
      const match = text(mergedValue(sheet, row, col)).match(
        /(?:\(?\s*)?(\d{1,2}[./-]\d{1,2}[./-]\d{4})(?:\s*\))?/,
      );
  ...
  for (const className of classes) {
    const rows = data.students
      ...
      .map((student, index) => [
        ...
        student.name,
        student.className,
        ...data.columns.map((col) => student.values[col.key]),
      ]);
